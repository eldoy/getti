...
it('valid url + undefined type, throws error', async ({ t, $ }) => {
  await t.rejects(
    async () => await $.getti('http://localhost:9090/json-sample', {}),
...
it('valid url + invalid callback, throws error', async ({ t, $ }) => {
  await t.rejects(
    async () => await $.getti('http://localhost:9090/json-sample.json', {}),
    new TypeError('Callback must be a function')
  )
})

it('valid url, returns results', async ({ t, $ }) => {
  var r = await $.getti('http://localhost:9090/json-sample.json')
  t.ok(r.length == 2)
})
